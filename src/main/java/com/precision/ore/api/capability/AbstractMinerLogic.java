...
public abstract class AbstractMinerLogic {
	...
	/** Performs the actual drilling Call this method every tick in update */
	public void performDrilling() {
		if (getMetaTileEntity().getWorld().isRemote) return;

		if (!getMetaTileEntity()
				.getAbilities(OreMultiblockAbility.DRILL_HOLDER)
				.get(0)
				.hasDrillHead()) return;
		...
		if (vein == null || getLayer() != previousLayer) {
			progressTime = 0;
			if (!acquireNewOre()) return; // stop if we still have no Ore
		}
		...
		// increase progress
		progressTime++;
		if (progressTime % getMaxProgressTime() != 0) return;
		progressTime = 0;

		List<ItemStack> outOres = new ArrayList<>();

		for (Material ore : vein.getDefinition().getStoredOres())
			if (GTValues.RNG.nextInt(vein.getDefinition().getAllOresWeight())
					<= vein.getDefinition().getOreWeight(ore))
		...
		if (getMetaTileEntity().fillInventory(outOres, true)) {
			...
			BedrockOreVeinHandler.depleteVein(
					getMetaTileEntity().getWorld(), getChunkX(), getChunkZ(), getLayer(), 1, false);
		} else {
	...
	protected abstract boolean checkCanDrain();

	protected abstract boolean consumeEnergy(boolean simulate);

	/** @return true if the rig is able to drain, else false */
	protected abstract boolean consumeFluid(boolean simulate);

	private boolean acquireNewOre() {
		this.vein = BedrockOreVeinHandler.getOreVeinWorldEntry(
				getMetaTileEntity().getWorld(), getChunkX(), getChunkZ(), getLayer());
