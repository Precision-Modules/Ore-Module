...
public class BasicMinerLogic extends AbstractMinerLogic {
	...
	protected boolean consumeFluid(boolean simulate) {
		if (vein.getDefinition().getSpecialFluids().isEmpty()) return true;
	...
	protected boolean checkCanDrain() {
		...
		if (this.hasNotEnoughEnergy
				&& getMetaTileEntity().getEnergyInputPerSecond()
						> 19L * GTValues.VA[getMetaTileEntity().getEnergyTier()]) {
	...
	public int getMaxProgressTime() {
		return (int) Math.max(
				2,
				super.getMaxProgressTime() * Math.pow(2, -(getMetaTileEntity().getEnergyTier() - 1)));
	}
}
