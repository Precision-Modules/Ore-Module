...
public class BedrockOreVeinHandler {
	...
	public static final LinkedHashMap<BedrockOreDepositDefinition, Integer> veinList = new LinkedHashMap<>();
	 ...
	 */
	@Nullable public static OreVeinWorldEntry getOreVeinWorldEntry(@Nonnull World world, int chunkX, int chunkZ, int layer) {
		if (world.isRemote) return null;

		ChunkPosDimension coords = new ChunkPosDimension(
				world.provider.getDimension(), chunkX / VEIN_CHUNK_SIZE, chunkZ / VEIN_CHUNK_SIZE);

		if (veinCache.contains(coords, layer)) return veinCache.get(coords, layer);

		BedrockOreDepositDefinition definition = null;
		OreVeinWorldEntry worldEntry;

		int query = world.getChunk(chunkX / VEIN_CHUNK_SIZE, chunkZ / VEIN_CHUNK_SIZE)
				.getRandomWithSeed(90210)
				.nextInt();
		...
		if (totalWeight > 0) {
			...
			for (Map.Entry<BedrockOreDepositDefinition, Integer> entry : veinList.entrySet()) {
				if (entry.getKey().getLayer() != layer) continue;
				int veinWeight = entry.getValue()
						+ entry.getKey().getBiomeWeightModifier().apply(biome);
	...
	public static BedrockOreDepositDefinition getDepositByName(String name) {
		if (nameVein.containsKey(name)) return nameVein.get(name);
	...
	public static void recalculateChances(boolean mutePackets) {
		...
		if (FMLCommonHandler.instance().getEffectiveSide() == Side.SERVER && !mutePackets) {
			...
			for (Table.Cell<ChunkPosDimension, Integer, OreVeinWorldEntry> entry :
					BedrockOreVeinHandler.veinCache.cellSet()) {
				if (entry.getRowKey() != null && entry.getValue() != null)
					packetMap.put(
							entry.getValue(), entry.getValue().getDefinition().getWeight());
			}
	 ...
	 */
	@Nullable public static List<Material> getOresInChunk(World world, int chunkX, int chunkZ, int layer) {
		OreVeinWorldEntry info = getOreVeinWorldEntry(world, chunkX, chunkZ, layer);
	 ...
	 */
	public static void depleteVein(
			World world, int chunkX, int chunkZ, int layer, int amount, boolean ignoreVeinStats) {
		OreVeinWorldEntry info = getOreVeinWorldEntry(world, chunkX, chunkZ, layer);
		if (info == null) return;
