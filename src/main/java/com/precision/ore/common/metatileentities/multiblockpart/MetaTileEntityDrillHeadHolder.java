...
public class MetaTileEntityDrillHeadHolder extends MetaTileEntityMultiblockPart
		implements IDrillHeadHolder, IMultiblockAbilityPart<IDrillHeadHolder> {
	...
	protected ModularUI createUI(EntityPlayer entityPlayer) {
		return ModularUI.builder(GuiTextures.BACKGROUND, 176, 166)
				.widget(new SlotWidget(drillHandler, 0, 79, 34).setBackgroundTexture(GuiTextures.SLOT))
	...
	public void dustParticles() {
		...
		getWorld()
				.spawnParticle(
						EnumParticleTypes.SMOKE_LARGE, x + 0.5F, getPos().getY() - 2, z + 0.5F, xSpd, ySpd, zSpd);
	}
	...
	public void renderMetaTileEntity(CCRenderState renderState, Matrix4 translation, IVertexOperation[] pipeline) {
		...
		if (getWorld() != null) {
			OreTextures.DRILL_HEAD_RENDERER.render(
					renderState,
					translation,
					pipeline,
					getPos().getY(),
	...
	private class InventoryDrillHandler extends ItemStackHandler {
		...
		@Nullable private ItemStack getDrillHeadStack() {
			if (!hasDrillHead()) return null;
			return getStackInSlot(0);
		}

		@Nullable private DrillHeadBehavior getDrillHeadBehavior() {
