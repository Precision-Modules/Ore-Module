...
public class ProspectorApp extends AbstractApplication implements SearchComponent.IWidgetSearch<String> {
	...
	private final gregtech.common.terminal.app.prospector.ProspectorMode mode;
	...
	public AbstractApplication initApp() {
		...
		if (isClient) {
			...
			this.widgetOreList =
					new WidgetOreList(32 * chunkRadius - 16, offset, 333 - 32 * chunkRadius + 16, 232 - 2 * offset);
			this.addWidget(this.widgetOreList);
		}
		this.widgetProspectingMap =
				new WidgetProspectingMap(0, offset + (7 - chunkRadius) * 16, chunkRadius, this.widgetOreList, mode, 1);
		if (isClient) {
		...
		if (isClient) {
			...
			this.addWidget(new LabelWidget(-2 + (16 * (chunkRadius * 2 - 1)) / 2, offset, "N", this::labelColor)
					.setShadow(true));
			this.addWidget(new LabelWidget(
							-2 + (16 * (chunkRadius * 2 - 1)) / 2,
							offset - 6 + 16 * (chunkRadius * 2 - 1),
							"S",
							this::labelColor)
					.setShadow(true));
			this.addWidget(new LabelWidget(0, offset - 3 + (16 * (chunkRadius * 2 - 1)) / 2, "W", this::labelColor)
					.setShadow(true));
			this.addWidget(new LabelWidget(
							-6 + 16 * (chunkRadius * 2 - 1),
							offset - 3 + (16 * (chunkRadius * 2 - 1)) / 2,
							"E",
							this::labelColor)
					.setShadow(true));
		}
						...
						() -> { // thread for better QoL
							...
							for (int i = playerChunkX - chunkRadius; i <= playerChunkX + chunkRadius; i++) {
								for (int j = playerChunkZ - chunkRadius; j <= playerChunkZ + chunkRadius; j++) {
									...
									try {
										nbt = CompressedStreamTools.read(new File(
												TerminalRegistry.TERMINAL_PATH,
												String.format(
														"%s/%d/%d_%d.nbt", getRegistryName(), mode.ordinal(), i, j)));
									} catch (IOException e) {
										GTLog.logger.error(
												"error while loading local nbt for {}", getRegistryName(), e);
									}
									if (nbt != null) {
						...
						() -> { // thread for better QoL
							...
							for (Table.Cell<Integer, Integer, PacketProspecting> cell : persist.cellSet()) {
								if (cell.getValue() != null) {
									...
									try {
										if (!nbt.isEmpty()) {
											CompressedStreamTools.safeWrite(
													nbt,
													new File(
															folder,
															String.format(
	...
	public List<IMenuComponent> getMenuComponents() {
		ClickComponent darkMode = new ClickComponent()
				.setIcon(GuiTextures.ICON_VISIBLE)
				.setHoverText("terminal.prospector.vis_mode")
				.setClickConsumer(cd -> {
					if (cd.isClient) {
	...
	public void search(String word, Consumer<String> find) {
		if (widgetOreList != null) {
			...
			for (Map.Entry<String, String> entry : widgetOreList.ores.entrySet()) {
				if (entry.getKey().toLowerCase().contains(word)
						|| entry.getValue().toLowerCase().contains(word)) {
