...
public class WidgetOreList extends DraggableScrollableWidgetGroup {
    ...
    private void addOre(String orePrefix) {
        ...
        BedrockOreDepositDefinition entry = BedrockOreVeinHandler.getDepositByName(orePrefix);

        if (entry == null) return;
        ores.put(orePrefix, entry.getAssignedName());
        ItemStack itemStack = OreDictUnifier.get(OrePrefix.crushed, entry.getStoredOres().get(0));
        ...
        widgetGroup.addWidget(new LabelWidget(20, 5, entry.getAssignedName(), materialStack == null ? orePrefix.hashCode() : materialStack.material.getMaterialRGB() | 0XFF000000));
            ...
            widgets.forEach(widget -> {
                if (widget instanceof WidgetGroup) {
                    ...
                    if (widget1 instanceof SlotWidget) {
                        ...
                        BedrockOreDepositDefinition deposit = BedrockOreVeinHandler.getDepositByName(widgetMap.get(widget));
                        if(deposit == null) return;

                        List<Material> ores = deposit.getStoredOres();

                        if (!ores.isEmpty()) {
                            slotWidget.getHandle().decrStackSize(64);
                            slotWidget.getHandle().putStack(OreDictUnifier.get(OrePrefix.crushed, ores.get(Math.floorMod(tickCounter / 20, ores.size()))));
                        }
                    }
                }
