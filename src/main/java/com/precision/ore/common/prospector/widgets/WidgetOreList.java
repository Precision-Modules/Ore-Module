...
public class WidgetOreList extends DraggableScrollableWidgetGroup {
	...
	private void addOre(String orePrefix) {
		...
		ItemStack itemStack =
				OreDictUnifier.get(OrePrefix.crushed, entry.getStoredOres().get(0));
	...
	private void addOil(String orePrefix) {
		...
		widgetGroup.addWidget(
				new LabelWidget(20, 5, fluidStack.getLocalizedName(), getFluidColor(fluidStack.getFluid())));
		addOrePrefix(orePrefix, widgetGroup);
	}
		...
		if (tickCounter % 20 == 0) {
			widgets.forEach(widget -> {
				if (widget instanceof WidgetGroup) {
					Widget widget1 =
							((WidgetGroup) widget).getContainedWidgets(true).get(0);
					if (widget1 instanceof SlotWidget) {
						SlotWidget slotWidget = (SlotWidget) widget1;
						BedrockOreDepositDefinition deposit =
								BedrockOreVeinHandler.getDepositByName(widgetMap.get(widget));
						...
						if (!ores.isEmpty()) {
							...
							slotWidget
									.getHandle()
									.putStack(OreDictUnifier.get(
											OrePrefix.crushed, ores.get(Math.floorMod(tickCounter / 20, ores.size()))));
						}
					}
				}
			});
