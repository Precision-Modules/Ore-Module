...
public class WidgetProspectingMap extends Widget {
	...
	public WidgetProspectingMap(
			int xPosition,
			int yPosition,
			int chunkRadius,
			WidgetOreList widgetOreList,
			@Nonnull ProspectorMode mode,
			int scanTick) {
	...
	public void detectAndSendChanges() {
		...
		if (FMLCommonHandler.instance().getMinecraftServerInstance().getTickCounter() % scanTick == 0
			...
			switch (mode) {
				case ORE:
					BedrockOreVeinHandler.OreVeinWorldEntry entry =
							BedrockOreVeinHandler.getOreVeinWorldEntry(world, chunk.x, chunk.z, 1);
					...
					break;
				case FLUID:
					BedrockFluidVeinHandler.FluidVeinWorldEntry fStack =
							BedrockFluidVeinHandler.getFluidVeinWorldEntry(world, chunk.x, chunk.z);
						...
						packet.setData(
								...
								TextFormattingUtil.formatNumbers(100.0
										* BedrockFluidVeinHandler.getOperationsRemaining(world, chunk.x, chunk.z)
										/ BedrockFluidVeinHandler.MAXIMUM_VEIN_OPERATIONS));
						packet.setData(
								2, String.valueOf(BedrockFluidVeinHandler.getFluidYield(world, chunk.x, chunk.z)));
	...
	public void drawInForeground(int mouseX, int mouseY) {
		...
		if (this.isMouseOverElement(mouseX, mouseY) && texture != null) {
			...
			if (cX >= chunkRadius * 2 - 1 || cZ >= chunkRadius * 2 - 1) return;
			// draw hover layer
			Gui.drawRect(
					cX * 16 + this.getPosition().x,
			...
			if (Loader.isModLoaded(GTValues.MODID_JOURNEYMAP)
					|| Loader.isModLoaded(GTValues.MODID_VOXELMAP)
	...
	public boolean mouseClicked(int mouseX, int mouseY, int button) {
		...
		if (cX >= chunkRadius * 2 - 1 || cZ >= chunkRadius * 2 - 1) return false;
	...
	private boolean addJourneymapWaypoint(BlockPos b) {
		journeymap.client.model.Waypoint journeyMapWaypoint = new journeymap.client.model.Waypoint(
				createVeinName(),
				b,
				new Color(color),
	...
	private boolean addVoxelMapWaypoint(@Nonnull BlockPos b) {
		...
		com.mamiyaotaru.voxelmap.interfaces.IWaypointManager waypointManager =
				com.mamiyaotaru.voxelmap.interfaces.AbstractVoxelMap.getInstance()
						.getWaypointManager();
	...
	private boolean addXaeroMapWaypoint(@Nonnull BlockPos b) {
		...
		xaero.common.minimap.waypoints.WaypointSet wps =
				minimapSession.getWaypointsManager().getWaypoints();
		xaero.common.minimap.waypoints.WaypointWorld ww =
				minimapSession.getWaypointsManager().getCurrentWorld();
		xaero.common.minimap.waypoints.Waypoint xaeroWaypoint = new xaero.common.minimap.waypoints.Waypoint(
				b.getX(),
				Minecraft.getMinecraft().world.getHeight(b.getX(), b.getZ()),
				b.getZ(),
				createVeinName(),
				hoveredNames.get(0).substring(0, 1),
				bestColorIndex);
