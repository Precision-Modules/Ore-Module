...
import com.precision.ore.common.prospector.widget.WidgetOreList;
import com.precision.ore.common.prospector.widget.WidgetProspectingMap;
...
public class ProspectorScannerBehavior implements IItemBehaviour, ItemUIFactory, SearchComponent.IWidgetSearch<String> {
    ...
    public ActionResult<ItemStack> onItemRightClick(@Nonnull World world, @Nonnull EntityPlayer player, EnumHand hand) {
        ...
        if (!world.isRemote) {
            if (player.isSneaking()) {
                ...
                Pair<Integer, Integer> nextMode = getNextMode(getMode(stack));
                if (nextMode.first() == WidgetProspectingMap.FLUID_PROSPECTING_MODE) {
    ...
    private Pair<Integer, Integer> getMode(ItemStack stack) {
        ...
        if (tag.hasKey("Layer", Constants.NBT.TAG_INT)) {
            layer = tag.getInteger("Layer");
        }
        return Pair.of(mode, layer);
    }

    private Pair<Integer, Integer> getNextMode(Pair<Integer, Integer> mode) {
        int l = mode.first();
        int r = mode.second();
        if(l == WidgetProspectingMap.ORE_PROSPECTING_MODE && r >= PrecisionWorldGenRegistry.getLayerOperations().size() - 1) {
            return Pair.of(tier >= FLUID_PROSPECTION_THRESHOLD ? WidgetProspectingMap.FLUID_PROSPECTING_MODE : WidgetProspectingMap.ORE_PROSPECTING_MODE, 0);
        } else if(l == WidgetProspectingMap.ORE_PROSPECTING_MODE){
            ++r;
        } else {
            return Pair.of(WidgetProspectingMap.ORE_PROSPECTING_MODE, 0);
        }

        return Pair.of(l, r);
    }

    private void setMode(ItemStack stack, Pair<Integer, Integer> mode) {
        if (!stack.hasTagCompound()) {
            stack.setTagCompound(new NBTTagCompound());
        }
        //noinspection ConstantConditions
        stack.getTagCompound().setInteger("Mode", mode.first());
        stack.getTagCompound().setInteger("Layer", mode.second());
    }
    ...
    public ModularUI createUI(PlayerInventoryHolder holder, @Nonnull EntityPlayer entityPlayer) {
        Pair<Integer, Integer> mode = getMode(entityPlayer.getHeldItem(EnumHand.MAIN_HAND));
        ...
        builder.widget(new WidgetProspectingMap(6, 18, radius, this.widgetOreList, mode.first(), mode.second(), 1));
    ...
    public void addInformation(ItemStack itemStack, List<String> lines) {
        ...
        if (tier >= FLUID_PROSPECTION_THRESHOLD) {
            ...
            lines.add(I18n.format(getMode(itemStack).first() == WidgetProspectingMap.ORE_PROSPECTING_MODE ? "metaitem.prospector.mode.ores" : "metaitem.prospector.mode.fluid"));
        } else {
