...
public class OreModuleJEIPlugin implements IModPlugin {
	...
	public void registerCategories(@Nonnull IRecipeCategoryRegistration registry) {
		registry.addRecipeCategories(
				new VirtualOreCategory(0, registry.getJeiHelpers().getGuiHelper()));
		registry.addRecipeCategories(
				new VirtualOreCategory(1, registry.getJeiHelpers().getGuiHelper()));
	}
	...
	public void register(@Nonnull IModRegistry registry) {
		...
		if (OreConfig.disableGregTechOreGeneration) {
			Set<OrePrefix> oreTypesToRemove =
					ImmutableSet.of(OrePrefix.ore, OrePrefix.oreNetherrack, OrePrefix.oreEndstone);

			if (ConfigHolder.worldgen.allUniqueStoneTypes) {
				oreTypesToRemove.addAll(ImmutableSet.of(
		...
		veinList.forEach((layer, list) -> {
			...
			registry.addRecipeCatalyst(
					layer == 0 ? PRIMITIVE_DRILL.getStackForm() : DRILL_HEAD.getStackForm(), veinSpawnID);
