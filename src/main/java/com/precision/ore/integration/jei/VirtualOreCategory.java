...
public class VirtualOreCategory extends CustomBasicRecipeCategory<VirtualOreInfo, VirtualOreInfo> {
	...
	public VirtualOreCategory(int layer, IGuiHelper guiHelper) {
		super("vein_info" + layer, "vein.info.name", guiHelper.createBlankDrawable(176, 166), guiHelper);
		...
		this.slot = guiHelper
				...
				.build();
		this.fluidSlot = guiHelper
				.drawableBuilder(GuiTextures.FLUID_SLOT.imageLocation, 0, 0, 18, 18)
				.setTextureSize(18, 18)
				.build();
	}
	...
	public void drawExtras(@Nonnull Minecraft minecraft) {
		...
		int maxOperations = BedrockOreVeinHandler.getOperationsPerLayer(definition.getLayer())
				.second();
		String maxOperationsStr = maxOperations >= 1000 ? maxOperations / 1000 + "k" : Integer.toString(maxOperations);

		int minOperations = BedrockOreVeinHandler.getOperationsPerLayer(definition.getLayer())
				.first();
		String minOperationsStr = minOperations >= 1000 ? minOperations / 1000 + "k" : Integer.toString(minOperations);

		// Create the Size
		minecraft.fontRenderer.drawString(
				"Size: " + minOperationsStr + " - " + maxOperationsStr, baseXPos, baseYPos, 0x111111);
		...
		// Will attempt to write dimension IDs in a single line, separated by commas. If the list is so long such that
		// it
		...
		for (int i = 0; i < dimensionIDs.size(); i++) {
			...
			if (namedDimensions.containsKey(dimensionIDs.get(i))) {
				...
				fullDimName = i == dimensionIDs.size() - 1 ? dimName : dimName + ", ";
			}
