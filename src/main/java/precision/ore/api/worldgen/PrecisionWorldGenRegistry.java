package precision.ore.api.worldgen;
...
public class PrecisionWorldGenRegistry {
    ...
    /**
     * Handles the setup of precision.ore generation files in the config folder.
    ...
    public void reinitializeRegisteredVeins() throws IOException {
        ...
        for (Path worldgenDefinition : veinFiles) {
            ...
            try {
                ...
                OreModule.logger.error("Failed to parse worldgen definition {} on path {}", depositName, worldgenDefinition, exception);
            }

        }

        addAddonFiles(worldgenRootPath, addonRegisteredBedrockOreVeinDefinitions, addonRegisteredBedrockOreVeinDefinitions);

        OreModule.logger.info("Loaded {} bedrock worldgen definitions", registeredBedrockOreVeinDefinitions.size());
        OreModule.logger.info("Loaded {} bedrock worldgen definitions from addon mods", addonRegisteredBedrockOreVeinDefinitions.size());
    }
    ...
    private static void extractJarVeinDefinitions(Path configPath, Path targetPath) throws IOException {
        ...
        try {
            URI sampleUri = PrecisionWorldGenRegistry.class.getResource("/assets/ore-module/").toURI();
            ...
            if (sampleUri.getScheme().equals("jar") || sampleUri.getScheme().equals("zip")) {
                ...
                worldgenJarRootPath = Paths.get(PrecisionWorldGenRegistry.class.getResource("/assets/ore-module/worldgen").toURI());
                oreVeinJarRootPath = Paths.get(PrecisionWorldGenRegistry.class.getResource("/assets/ore-module/worldgen/vein").toURI());
            } else {
            ...
            if (targetPath.compareTo(oreVeinRootPath) == 0) {
                OreModule.logger.info("Attempting extraction of standard worldgen definitions from {} to {}",
                ...
                OreModule.logger.info("Extracted {} builtin worldgen vein definitions into vein folder", jarFiles.size());
            }
            // Attempts to extract the named dimensions json folder
            else if (targetPath.compareTo(dimensionsRootPath) == 0) {
                OreModule.logger.info("Attempting extraction of standard dimension definitions from {} to {}",
                ...
                OreModule.logger.info("Attempting extraction of standard layers definitions from {} to {}",
    ...
    private <T extends IWorldgenDefinition> void addAddonFiles(Path root, @Nonnull List<T> definitions, @Nonnull List<T> registeredDefinitions){
        for(IWorldgenDefinition definition : definitions) {
            ...
            if(element == null) {
                OreModule.logger.error("Addon mod tried to register bad precision.ore definition at {}", definition.getDepositName());
            ...
            try {
                ...
                OreModule.logger.error("Failed to parse addon worldgen definition {}", definition.getDepositName(), exception);
            }
        }
    }
