package precision.ore.common.prospector.widget;
...
import precision.ore.common.prospector.ProspectingTexture;
...
public class WidgetProspectingMap extends Widget {
    ...
    public WidgetProspectingMap(int xPosition, int yPosition, int chunkRadius, WidgetOreList widgetOreList, int mode, int layer, int scanTick) {
        ...
        if (FMLCommonHandler.instance().getMinecraftServerInstance().getTickCounter() % scanTick == 0 && chunkIndex < (chunkRadius * 2 - 1) * (chunkRadius * 2 - 1)) {
            ...
            switch (mode) {
                case ORE_PROSPECTING_MODE:
                    BedrockOreVeinHandler.OreVeinWorldEntry oreStack = BedrockOreVeinHandler.getOreVeinWorldEntry(world, chunk.x, chunk.z, layer);
                    if (oreStack != null && oreStack.getDefinition() != null) {
                        packet.addBlock(0, 3, 0, GTUtility.formatNumbers(100.0 * oreStack.getOperationsRemaining() / BedrockOreVeinHandler.getOperationsPerLayer(layer).second()));
                        packet.addBlock(0, 2, 0, "" + oreStack.getOperationsRemaining());
                        packet.addBlock(0, 1, 0, oreStack.getDefinition().getDepositName());
                    }
                    break;
    ...
    public void drawInForeground(int mouseX, int mouseY) {
        ...
        if (this.isMouseOverElement(mouseX, mouseY) && texture != null) {
            ...
            if (this.mode == 0) { // draw precision.ore
                tooltips.add(I18n.format("terminal.prospector.precision.ore"));
                    ...
                    if (texture.getSelected().equals("[all]") || texture.getSelected().equals(texture.map[cX][cZ].get((byte) 1))) {
                        ...
                        if(definition != null) {
                            ...
                            if(Integer.parseInt(yieldSize) > 1) {
                                tooltips.add(I18n.format("terminal.prospector.precision.ore.info_yield",
                                        definition.getAssignedName(),
                                        yieldSize,
                                        texture.map[cX][cZ].get((byte) 3)));
                            } else {
                                tooltips.add(I18n.format("terminal.prospector.precision.ore.info",
                                        definition.getAssignedName(),
                                        texture.map[cX][cZ].get((byte) 3)));
                            }
                        }
